...
Compared to pure GravityCompController, this adds two IMU-driven terms:

1. **Swing-leg gravity boost**: When a leg is in swing phase (no ground
   contact), its joints receive extra gravity compensation.  During stance,
   GRF partially counteracts gravity, so the default GC is already an
   "over-compensation" that benefits learning.  During swing, there is no
   GRF support, so additional compensation makes the swing leg lighter
   and easier to control.

2. **Disturbance correction**: When the IMU residual acceleration exceeds
   a threshold (indicating an unexpected disturbance, not steady-state
   contact), a correction torque is applied via the pelvis site Jacobian.
   This is gated to avoid weakening the beneficial stance over-compensation.

Neither term uses qacc estimation or privileged simulator information.
All signals come from IMU (accelerometer) + joint encoders + gait phase.
"""
...
class IMUGravityCompController(MotorController):
    """PD control with IMU-enhanced gravity compensation.

    τ = kp*(q_d - q) - kd*q̇
        + gravity_scale * g(q) * (gravity_comp_mask + swing_boost * swing_mask_per_env)
        + disturbance_scale * τ_disturbance
    """
    ...
    def compute(
        ...
        *,
        ...
        tau_disturbance: np.ndarray | None = None,
        **kwargs,
    ) -> np.ndarray:
        """Compute PD + IMU-enhanced gravity compensation torques.
        ...
        Args:
            ...
            swing_mask_per_env: Per-env swing leg mask ``(num_envs, num_actions)``.
                ...
                Computed externally from gait phase + IMU cross-validation.
            tau_disturbance: Disturbance correction torque ``(num_envs, num_actions)``.
                Computed externally from IMU residual acceleration, gated
                by a threshold to avoid weakening stance over-compensation.
